...
export default function DatabaseSelector() {
  ...
  const handleSingle = (qId, optId, optScores) => {
    setAnswers((prev) => ({ ...prev, [qId]: [optId] }));
  };

  const handleMulti = (qId, optId) => {
    setAnswers((prev) => {
        ...
        <div style={styles.card}>
          ...
          <div style={styles.dbGrid}>
            {Object.values(databases).slice(0, 10).map((db) => (
              <div key={db.name} style={{ ...styles.dbChip, background: db.light, border: `1.5px solid ${db.color}22`, color: db.color }}>
                <span>{db.emoji}</span> {db.name}
              </div>
            ))}
            <div style={{ ...styles.dbChip, background: "#f3f4f6", border: "1.5px dashed #9ca3af", color: "#6b7280" }}>
              +5 more
            </div>
          </div>
      ...
      <div style={styles.root}>
        <div style={{ ...styles.card, maxWidth: 780 }}>
            ...
            {results.map(([dbKey, score], i) => {
              ...
              return (
                <div
                  ...
                  style={{
                    ...styles.resultCard,
                    border: isTop ? `2px solid ${db.color}` : "1.5px solid #e5e7eb",
                    background: isTop ? db.light : "#fafafa",
                ...
                >
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, margin: "10px 0 6px" }}>
                    {db.tags.map((t) => (
                      <span key={t} style={{ ...styles.tag, background: db.light, color: db.color, border: `1px solid ${db.color}33` }}>{t}</span>
                    ))}
                  </div>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 24, flexWrap: "wrap" }}>
            ...
            <button
              style={{ ...styles.startBtn, background: "#f3f4f6", color: "#374151", border: "1.5px solid #d1d5db" }}
              onClick={() => window.open("https://academiapilogt.com", "_blank")}
            >
              Read Full Guide →
            </button>
          </div>
    ...
    <div style={styles.root}>
      <div style={styles.card}>
          ...
          {q.options.map((opt) => {
            ...
            return (
              <button
                ...
                style={{
                  ...styles.optionBtn,
                  ...(sel ? { borderColor: sectionColor, background: `${sectionColor}10`, color: "#111827" } : {}),
                }}
                onClick={() => {
              ...
              >
                <span style={{ ...styles.optionCheck, borderColor: sel ? sectionColor : "#d1d5db", background: sel ? sectionColor : "white" }}>
        ...
        <div style={styles.navRow}>
          ...
          {step < totalSteps ? (
            <button
              style={{ ...styles.nextBtn, background: canProceed() ? sectionColor : "#d1d5db", cursor: canProceed() ? "pointer" : "not-allowed" }}
            ...
            <button
              style={{ ...styles.nextBtn, background: canProceed() ? "#059669" : "#d1d5db", cursor: canProceed() ? "pointer" : "not-allowed" }}
        ...
        <div style={styles.dotsRow}>
          {questions.map((_, i) => (
            <div
              ...
              style={{
                ...styles.dot,
                background: i + 1 < step ? sectionColor : i + 1 === step ? sectionColor : "#e5e7eb",
                width: i + 1 === step ? 20 : 8,
